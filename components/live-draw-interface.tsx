...
export function LiveDrawInterface({ raffle, prizes, participants, existingWinners }: LiveDrawInterfaceProps) {
  ...
  useEffect(() => {
    if (raffle.status === "upcoming" && !isRaffleLive) {
      setRaffleToLive()
    }
  }, [])

  const setRaffleToLive = async () => {
    await supabase.from("raffles").update({ status: "live" }).eq("id", raffle.id)
